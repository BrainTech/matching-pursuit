...
/* MMP3 - MULTICHANNEL MATCHING PURSUIT */

void firstIterationMMP3(MP5Parameters *mp5Parameters, DataParameters *dataParameters, GaborDictionary *gaborDictionary)
{
	...
	*(mp5Parameters->signalEnergyInEachChannel + channel) = findSignalEnergy(signalTable,dimOffset);
...
void nextIterationMMP3(MP5Parameters *mp5Parameters, DataParameters *dataParameters, GaborDictionary *gaborDictionary)
{
    unsigned long  int stepInToolbar, step = 0;
    unsigned short int channel;
    unsigned short int numberOfAnalysedChannels = mp5Parameters->numberOfAnalysedChannels;
